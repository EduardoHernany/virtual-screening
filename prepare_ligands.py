...
def process_sdf_with_obabel(sdf_path, output_dir=None):
    ...
    cmd = [
        ...
        "-isdf", str(sdf_path),
        "-opdbqt",
        "-O", str(output_dir /  "mol.pdbqt"),  # Template de saída
...
Comparação:
  ...
  OpenBabel: Mais rápido, conversão simples, menos otimizado (~0.1-0.5 seg/mol)
    ...
    parser.add_argument(
        'sdf_file',
        help='Caminho para o arquivo SDF contendo os ligantes'
    )
    ...
    if stats.get('arquivos_gerados'):
        print(f"\n💡 Próximo passo:")
        print(f"   python autodock_gpu_screening.py receptor.pdbqt {args.output_dir or 'ligands_pdbqt'}/")
        sys.exit(1)
